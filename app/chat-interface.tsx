...
export function ChatInterface({
  ...
  const handleRewrite = () => {
    ...
    if (lastUserMessage) {
      handleInputChange({
        target: { value: getMessageContent(lastUserMessage) },
      } as React.ChangeEvent<HTMLTextAreaElement>);
      // Submit the form
      setTimeout(() => {
        formRef.current?.requestSubmit();
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col relative">
            ...
            {messages.length > 2 && (
              <>
                ...
                {(() => {
                  ...
                  const messageFollowUpQuestions =
                    storedData?.followUpQuestions || [];
                              ...
                              <div className="flex items-center gap-1">
                                <button
                                  onClick={() =>
                                    handleCopy(
                                      pair.assistant
                                        ? getMessageContent(pair.assistant)
                                        : "",
                                      `message-${pairIndex}`
                                    )
                                  }
                          ...
                          >
                            ...
                            <div className="relative p-3 flex flex-col justify-between h-full">
                              ...
                              <div className="flex items-center gap-1.5">
                                ...
                                <p className="text-[10px] text-gray-600 dark:text-gray-300 truncate flex-1 font-medium">
                                  {result.siteName ||
                                    new URL(result.url).hostname.replace(
                                      "www.",
                                      ""
                                    )}
                                </p>
        ...
        {/* Fixed input at bottom - Enhanced styling */}
        <div className="fixed lg:absolute bottom-0 left-0 right-0 bg-gradient-to-t from-white via-white/95 dark:from-zinc-900 dark:via-zinc-900/95 to-transparent pt-4 pb-4 sm:pt-6 sm:pb-6 z-30 backdrop-blur-sm">
          <div className="max-w-2xl mx-auto px-3 sm:px-4 lg:px-8">
            <form onSubmit={handleFormSubmit} ref={formRef}>
              <div className="bg-white/90 dark:bg-zinc-800/90 backdrop-blur-md rounded-2xl shadow-2xl border-2 border-gray-200 dark:border-gray-700 p-3 focus-within:border-orange-400 dark:focus-within:border-orange-500 focus-within:shadow-orange-100 dark:focus-within:shadow-orange-900/20 transition-all duration-300 hover:border-gray-300 dark:hover:border-gray-600">
      ...
      {(newsResults.length > 0 ||
        ...
        <div className="hidden lg:block w-80 min-w-[320px] bg-transparent overflow-y-auto p-4 space-y-6 scrollbar-hide">
          ...
          <ImageResults
            ...
            isLoading={isLoading && imageResults.length === 0}
          />

          {/* News Results */}
          <NewsResults
            results={newsResults}
            isLoading={isLoading && newsResults.length === 0}
          />
        </div>
      )}
    </div>
